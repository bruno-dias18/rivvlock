-- Security hardening: Address remaining security warnings (SAFE VERSION)
...
-- Create function to purge old activity logs (CREATE OR REPLACE is safe)
...
-- Create audit table for stripe_accounts access (IF NOT EXISTS)
...
-- Create function to log stripe account access (CREATE OR REPLACE is safe)
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE tgname = 'stripe_account_access_trigger'
  ) THEN
    CREATE TRIGGER stripe_account_access_trigger
    AFTER UPDATE ON public.stripe_accounts
    FOR EACH ROW
    EXECUTE FUNCTION public.log_stripe_account_access();
  END IF;
END $$;
