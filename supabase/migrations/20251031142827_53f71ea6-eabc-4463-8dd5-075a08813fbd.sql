-- Drop and recreate get_transaction_by_token_safe with new payment deadline fields
DROP FUNCTION IF EXISTS public.get_transaction_by_token_safe(text);
...
RETURNS TABLE(
  ...
  payment_deadline_card timestamp with time zone,
  payment_deadline_bank timestamp with time zone,
...
BEGIN
  ...
  -- Return safe subset of data including new payment deadline fields
  RETURN QUERY
  SELECT 
    v_transaction.id,
    v_transaction.title,
    ...
    v_transaction.payment_deadline_card,
    v_transaction.payment_deadline_bank,
