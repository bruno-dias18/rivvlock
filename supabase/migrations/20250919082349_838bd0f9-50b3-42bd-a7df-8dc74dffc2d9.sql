-- Security Fix: Add proper admin access policies for profiles table
...
-- Security Fix: Create a more secure admin role function that doesn't expose emails
...
-- Update the is_admin function to be more secure (remove email exposure)
...
-- Only super admin can manage admin roles (using user_id instead of email)
CREATE POLICY "Super admin can manage admin roles" ON public.admin_roles
FOR ALL 
USING (EXISTS (
  SELECT 1 FROM public.admin_roles 
  WHERE user_id = auth.uid() 
  AND user_id = 'f563ef63-1ae5-4108-8284-29a7dfee1741'::uuid -- Replace with actual super admin user_id
))
WITH CHECK (EXISTS (
  SELECT 1 FROM public.admin_roles 
  WHERE user_id = auth.uid() 
