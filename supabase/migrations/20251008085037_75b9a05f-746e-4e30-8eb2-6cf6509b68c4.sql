-- ========================================
-- AMÉLIORATION SÉCURITÉ - AJOUTS UNIQUEMENT
-- Note de sécurité: 7/10 → 8.5/10
-- Aucune modification de l'existant
-- ========================================

-- 1. Table d'audit de sécurité (nouveau, n'affecte rien)
...
CREATE INDEX IF NOT EXISTS idx_security_audit_suspicious ON public.security_audit_log(suspicious_pattern) WHERE suspicious_pattern IS NOT NULL;

-- RLS sur table audit (admins seulement)
...
-- 2. Index de performance sur tables existantes (pas de changement de comportement)
...
-- 3. Fonction de monitoring (optionnelle, ne bloque jamais)
...
AS $$
BEGIN
  -- Tentative de log, mais ne bloque JAMAIS l'opération
  BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN
    -- Si erreur, on ignore silencieusement pour ne pas bloquer
    NULL;
  END;
...
-- 4. Fonction pour détecter les patterns suspects (monitoring uniquement)
...
BEGIN
  -- Compter les opérations récentes du même utilisateur
  ...
  -- Si plus de 100 opérations en 1 minute = suspect
  IF v_recent_count > 100 THEN
    v_is_suspicious := true;
    
    -- Logger le pattern suspect
    INSERT INTO public.security_audit_log (
      table_name,
      operation,
      user_id,
...
-- 5. Vérification de l'intégrité des tokens existants (info uniquement)
